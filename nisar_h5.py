...
class HttpRangeFile(io.RawIOBase):
    """A seekable file-like object over HTTP range requests.

    Reads are served from a block cache, so HDF5's many small metadata reads
    collapse into a handful of round trips. `stats` reports what it actually
    cost, which is the number worth watching on a metered or slow link.
    """
...
def open_h5(uri: str, block: int = 1 << 20):
    ...
    if uri.startswith(("http://", "https://")):
        backing = HttpRangeFile(uri, block=block)
